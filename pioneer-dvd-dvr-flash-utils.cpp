...
// Handy macro for exiting. xbuffer or fd = nullptr is no problemo
// (except for lousy Visual C++, that will CRASH on fd = nullptr!!!!)
#define FREE_BUFFERS                                                                               \
    {                                                                                              \
        free(fbuffer[0]);                                                                          \
        free(fbuffer[1]);                                                                          \
...
u32 getSense(Scsi *scsi, char *errmsg = nullptr) {
    // Sense variables
    u32 s = 0xFFFFFFFF;
    char sense[20];
...
u8 pseudoRandom() {
    seed = ((((seed & 0xFFFF) * 0x41C6) + ((seed >> 16) * 0x4E6D)) << 16) +
           ((seed & 0xFFFF) * 0x4E6D) + 0x3039;
    return ((u8)(seed >> 16));
}
...
int Downgrade() {
    ...
    if ((dbuffer = (u8 *)calloc(0x400, 1)) == nullptr) {
        if (opt_yes)
    ...
    if (stat) { // Stat has to be right this time
    ...
    match = 0;
    for (s = 0; s < 0x10000; s++) { // Try the 65536 possible seeds
        seed = s;
        for (i = 0; i < 4; i++) {
...
int isDVD(Scsi *scsi) {
    ...
    if (result < 0)
        return 0;
    if (mbuffer[8] != 0x2A)
        return 0; // Security check: code page should be returned
...
int getInquiry(Scsi *scsi, char *vendor, char *model, char *revision) {
    ...
    memmove(vendor, mbuffer + 8, 8);
    vendor[8] = 0;
    memmove(model, mbuffer + 8 + 8, 16);
    model[16] = 0;
    memmove(revision, mbuffer + 8 + 8 + 16, 4);
    revision[4] = 0;
    ...
    if (getRPC(scsi, rpc_info) && rpc_info[6]) {
        ...
        if (rpc_info[6] == 1) {
            ...
            printf("                 %lu vendor reset%s remaining\n",
                   (unsigned long)((rpc_info[4] >> 3) & 0x07),
                   (((rpc_info[4] >> 3) & 0x07) > 1) ? "s" : "");
...
int main(int argc, char *argv[]) {
    ...
    scsi = newScsi();
    if (!scsi) {
        strncpy(str, "Internal error: newScsi() returned nullptr.\n", 42);
        if (opt_yes)
            printf("ERROR - %s", str);
        else
            fprintf(stderr, str);
    ...
    if ((mbuffer = (u8 *)calloc(MODE_SIZE, 1)) == nullptr) {
    ...
    if (!detected) {
        ...
        for (;;) {
            if (scsiReadName(scsi, devname, sizeof(devname)) < 0)
                break;
    ...
    if (((fbuffer[0] = (u8 *)calloc(MAX_SIZE, 1)) == nullptr) ||
        ((fbuffer[1] = (u8 *)calloc(MAX_SIZE, 1)) == nullptr)) {
    ...
    for (i = 0; i < nb_firmwares; i++) {
        if ((fd = fopen(fname[i], "rb")) == nullptr) {
            if (opt_verbose)
                perror("fopen()");
        ...
        if ((read > MAX_SIZE) ||
            (read < A06K_SIZE)) // 2.1 - Just check if file size is within range.
        {
            if (opt_verbose)
                perror("fread()");
    ...
    stat = scsiSay(scsi, (char *)cdb, 6, (char *)mbuffer, 0x60, X1_DATA_IN);
    if (stat < 0) { // negative stat indicates an error
    ...
    //	idx.Kernel_Size = A06K_SIZE;	//  2.1: Set later because of various firmware sizes for
    // DVR-109 	idx.Normal_Size = FIRM_SIZE;
    ...
    case 1:
        ...
        if (ftype[0] == FTYPE_KERNEL) {
            ...
            if (!opt_force)
                ...
                if ((((idx.Generation <= 8) ||
                      ((idx.Generation >= 52) && (idx.Generation <= 62))) &&
                     (idx.Kernel_Size != A06K_SIZE)) ||
                    ((((idx.Generation >= 9) && (idx.Generation <= 11)) ||
                      (idx.Generation >= 64)) &&
                     (idx.Kernel_Size != A09K_SIZE))) {
            ...
            if ((((idx.Generation <= 8) || ((idx.Generation >= 52) && (idx.Generation <= 62))) &&
                 (idx.Normal_Size != FIRM_SIZE)) ||
                ((((idx.Generation >= 9) && (idx.Generation <= 11)) || (idx.Generation >= 64)) &&
                 (idx.Normal_Size < A09F_SIZE))) {
        ...
        if ((strncmp((char *)fbuffer[0] + 0x60, id.Desc, 24)) ||
            ...
            if (opt_force < 2) {
                strncpy(str,
                        "If you want to convert your drive, you need to supply both Kernel and "
                        "General\n",
                        79);
                if (opt_yes)
                    printf("ERROR - %s", str);
    ...
    case 2:
        ...
        if ((((idx.Generation <= 8) || ((idx.Generation >= 52) && (idx.Generation <= 62))) &&
             (idx.Normal_Size != FIRM_SIZE) && (idx.Kernel_Size != A06K_SIZE)) ||
            ((((idx.Generation >= 9) && (idx.Generation <= 11)) || (idx.Generation >= 64)) &&
             (idx.Normal_Size < A09F_SIZE) && (idx.Kernel_Size != A09K_SIZE))) {
    ...
    if (nb_firmwares > 0) { // Additional tests
        ...
        if (idx.Generation == 8 && opt_force < 2) {
            if ((!strncmp(idx.Kernel_Type, "PIO_ADV", 7)) ||
                ...
                if (((strncmp((char *)fbuffer[0] + 0xd0, "PIO_ADV", 7)) &&
                     (!strncmp(idx.Kernel_Type, "PIO_ADV", 7)))) {
                    printf(
                        "You are trying to flash an standard DVR firmware onto what appears to\n");
                    printf(
                        "be an XL drive. Doing so can damage your drive: operation cancelled.\n");
                    ERR_EXIT;
                }
            }
        }
        ...
        if (!is_kernel &&
            (idx.Generation != 1)) { // No way we can check this if we are already in Kernel
            memset(cdb, 0x00, MAX_CDB_SIZE);
            // 1.3: cdb length must be 6 for TEST UNIT READY!
            stat = scsiSay(scsi, (char *)cdb, 6, nullptr, 0, X0_DATA_NOT);
            ...
            if ((!stat) || ((getSense(scsi) & 0xFFFF00) != 0x023A00)) {
                ...
                cdb[0] = 0x1B;
                cdb[4] = 0x02;
                scsiSay(scsi, (char *)cdb, 6, nullptr, 0, X0_DATA_NOT);
                ERR_EXIT;
            }
            TickleDrive(scsi);
        }
    ...
    if (((nb_firmwares > 0) || (opt_kernel)) && (!is_kernel)) {
        ...
        if (!opt_debug) {
            ...
            if (stat) { // Let's try with Generic key
    ...
    if ((kern_id != -1)) {
        ...
        for (i = 0; i < idx.Kernel_Size; i += 0x8000) {
            ...
            if (!opt_debug) {
                stat = scsiSay(scsi, (char *)cdb, 10, (char *)fbuffer[kern_id] + i, 0x8000,
                               X2_DATA_OUT);
                if (stat) { // Stat must be OK
    ...
    if ((norm_id != -1)) {
        ...
        for (i = 0; i < idx.Normal_Size; i += 0x8000) {
            ...
            if (!opt_debug) {
                stat = scsiSay(scsi, (char *)cdb, 10, (char *)fbuffer[norm_id] + i, 0x8000,
                               X2_DATA_OUT);
                if (stat) { // Stat must be OK
        ...
        if (idx.Generation >= 4) { // For some reason, on the 105/106/107 we need to wait after the
                                   // Normal data has been sent
