...
int sgioLimitSeconds(Sgio *sgio, int s, int ns) {
    ...
    if (ms < 0)
        ms = ((28 * 60 * 60) * 1000); /* 28 * 60 * 60 s = 28 hours = 1.125 days */
...
static int sgioSetCommand(Sgio *sgio, char const *cdbChars, int cdbLength, char *dataChars,
    ...
    if (UCHAR_MAX < cdbLength)
        return -1; /* UCHAR_MAX = max cmd_len */
    if (UCHAR_MAX < maxSenseLength)
        return -1; /* UCHAR_MAX = max mx_sb_len */
...
int sgioSay(Sgio *sgio, char const *cdbChars, int cdbLength, char *dataChars, int maxLength,
    ...
    int maxSenseLength = sgio->theMaxSenseLength;
    int exitInt        = sgioSetCommand(sgio, cdbChars, cdbLength, dataChars, maxLength, direction);
    if (exitInt != 0)
        return exitInt;
    ...
    if (exitInt == -1) {
        FILE *fi = sgio->theErrFile;
        (void)fprintf(fi, "// sgioSay x %lX %lX %lX %lX (ioctl sihInfo sihResid sihSbLenWr)\n",
...
int sgioReadName(Sgio *sgio, char *chars, int charsLength) {
    ...
    if ((MAX_DEV_HD + MAX_DEV_SG + MAX_DEV_SCD) <= readNameInt)
        return -1;
